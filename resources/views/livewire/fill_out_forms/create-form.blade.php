<div class="modal fade" id="create-form" data-backdrop="static" wire:ignore.self>
    <div class="modal-dialog">
        <div class="modal-content" x-data="{ form: '' }">
            <div wire:loading wire:target='createForm'>
            ...
            <form wire:submit.prevent='createForm(form)'>
                <div class="modal-body" style="max-height: 500px; overflow-y: auto; margin-left: 1rem; margin-right: 1rem;">
                    <!--MODAL FORM TITLE-->
                    <p class="card-title" style="color: #080743; font-size: 20px; font-weight: bold;">Create Form</p> <br><br>
                    ...
                    <template x-if="form == 'Violation Form'">
                        @include('livewire.fill_out_forms.create-violation-form')
                    </template>
                    <template x-if="form == 'Home Visitation Form'">
                        @include('livewire.fill_out_forms.create-home-visitation-form')
                    </template>
                </div> <!-- /.modal-body -->
                <div class="modal-footer justify-content-center">
                    <template x-if="form == 'Violation Form'">
                        <div class="col-md-10 offset-md-1 text-center">
                            <button class="btn btn-block btn-default" style="border-color: transparent; background-color: #0A0863; color: #252525; color:white;" type="submit" x-on:click="@this.set('studentsInvolve', studentsInvolve); @this.set('selectedTeacher', selectedTeacher)">Send</button>
                        </div>
                    </template>
                    <template x-if="form == 'Home Visitation Form'">
                        <div class="col-md-10 offset-md-1 text-center">
                            <button class="btn btn-block btn-default" style="border-color: transparent; background-color: #0A0863; color: #252525; color:white;" type="submit" x-on:click="@this.set('selectedStudent', selectedStudent); @this.set('selectedTeacher', selectedTeacher)">Send</button>
                        </div>
                    </template>
                </div>
            </form>
